...
Original file is located at
    https://colab.research.google.com/drive/1GkQz5My4p8cwdY9a09Q5PrqiZ0P4LS0g
"""
...
csv_file_path = "/content/drive/My Drive/Assignment_CSV/sector/sector_data.csv"
...
# saturday and sunday entire is there
import pandas as pd
from sqlalchemy import create_engine
...
try:
    # Query to check for Saturday and Sunday entries
    query = f"""
        SELECT date, DAYOFWEEK(date) as day_of_week
        FROM {tidb_table}
        WHERE DAYOFWEEK(date) IN (1, 7);
    """  # 1 represents Sunday, 7 represents Saturday

    df_weekend = pd.read_sql_query(query, engine)

    if not df_weekend.empty:
        print("Weekend dates found in the stock_data table:")
        print(df_weekend)
    else:
        print("No weekend entries found in the stock_data table.")
...
st.sidebar.info(
    ...
    "👩‍💻 Developed by **Mubina**  \n"
    "📡 Data Source: TiDB  \n"
    "🔗 [GitHub Repository](https://github.com/SMMubina/Data-Driven-Stock-Analysis)")
